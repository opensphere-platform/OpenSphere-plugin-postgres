...
test('PostgreSQL plugin owns the external Foundation-hosted custom element', () => {
  ...
  assert.equal(manifest.contributions.api.basePath, manifest.apiBase);
...
test('API contribution is the single package authority while governed writes remain on Foundation', () => {
  const packager = read('tools/package-module.mjs');
  const manifest = JSON.parse(read('ui-shell/ui-shell.manifest.json'));
  assert.equal(manifest.apiBase, '/api/plugins/postgres');
  assert.equal(manifest.contributions.api.enabled, true);
  assert.equal(manifest.contributions.api.basePath, manifest.apiBase);
  assert.doesNotMatch(packager, /\n\s+api:\s*manifest\.apiBase/);
  assert.match(packager, /contributions: manifest\.contributions/);
});
...
test('PostgreSQL separates selected-runtime navigation from management workspaces', () => {
  ...
  assert.match(css, /\.pgp-page-frame\s+\.pfs-plugin-head\s*\{[^}]*grid-template-columns:\s*minmax\(11\.5rem,\s*0\.5fr\)\s+minmax\(0,\s*1\.5fr\)[^}]*gap:\s*10px/);
  ...
  assert.match(css, /\.pgp-management-actions--header \.pgp-management-action os-cicon\s*\{[^}]*width:\s*16px[^}]*min-width:\s*16px[^}]*height:\s*16px[^}]*min-height:\s*16px/);
...
test('runtime Extensions and Parameters use native horizontal tabs without nested scrolling', () => {
  ...
  assert.doesNotMatch(extensions, /\.pge-table-wrap\{[^}]*overflow:auto/);
...
test('provisioning is namespace-first and uses one canonical PostgresClaim v1beta1 flow', () => {
  ...
  assert.match(fleet, /\/api\/foundation\/postgres\/claims/);
  assert.match(fleet, /reason: draft\.reason, confirm: draft\.name/);
  assert.doesNotMatch(component, /<pg-claims/);
  assert.match(component, /apiVersion: provisioning\.opensphere\.io\/v1beta1/);
  assert.doesNotMatch(fleet, /method: 'POST'.*provisioning\.opensphere\.io\/v1beta1\/namespaces/s);
  ...
  assert.match(fleet, /extensions: draft\.extensions/);
...
test('Profile Catalog is namespace scoped and keeps StackGres objects authoritative', () => {
  ...
  assert.doesNotMatch(catalog, /profileReason|deleteProfileReason|변경 사유|삭제 사유/);
  assert.doesNotMatch(fleet, /PostgresProfileDraft[\s\S]{0,250}reason:/);
