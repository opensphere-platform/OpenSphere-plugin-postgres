...
if (manifest.contributions?.api?.enabled !== true) throw new Error('plugin runtime API contribution must be enabled');
if (manifest.contributions?.api?.basePath !== manifest.apiBase) throw new Error('manifest API contribution mismatch');
if (descriptor.api !== undefined) throw new Error('legacy descriptor.api authority must be absent');
if (descriptor.contributions?.api?.basePath !== manifest.apiBase) throw new Error('descriptor API contribution mismatch');
